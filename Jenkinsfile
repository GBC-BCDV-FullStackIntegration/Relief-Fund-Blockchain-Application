      ...
      steps {
        script {
          docker.withRegistry("https://${env.ACR_LOGIN_SERVER}", env.ACR_CREDENTIALS_ID) {
            sh '''
              docker buildx build --platform linux/amd64,linux/arm64 -t ${env.ACR_LOGIN_SERVER}/backend:latest -f relief-fund-backend/Dockerfile --push .
              docker buildx build --platform linux/amd64,linux/arm64 -t ${env.ACR_LOGIN_SERVER}/frontend:latest -f relief-fund-frontend/Dockerfile --push .
            '''
          }
        }
      }
